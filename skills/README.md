...
| [bluefin-ci](bluefin-ci/SKILL.md) | The GitHub Actions pipeline ([`../.github/workflows/bluefin.yml`](../.github/workflows/bluefin.yml)) and the throughput regression gate ([`../bench_ci.sh`](../bench_ci.sh)): floors, env-var knobs (incl. `BLUEFIN_BENCH_GOOD_CONN_MIN_BYTES`), sticky PR comment, log artifacts, the 75 %-of-median ratchet protocol, and the fallback ladder when CI flakes. Load whenever a task asks "why did CI fail?", "how do I bump the gate?", or touches the workflow file / `bench_ci.sh`. Pair with `bluefin-performance` for the measurement side. |
| [bluefintool](bluefintool/SKILL.md) | The netcat-like CLI (`bluefintool`) for interactive smoke-testing and data piping over Bluefin. Load when you want to quickly verify connectivity, send a few bytes, or pipe data through a Bluefin tunnel without touching the bench harness. |
