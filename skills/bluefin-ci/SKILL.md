...
| `BLUEFIN_CLIENT_SEND_THROTTLE_US` | `0` (production / dev) / `50` (workflow) | Inter-batch pacing knob read by [`bluefin/src/bin/client.rs`](../../bluefin/src/bin/client.rs). When >0, the client send loop sleeps that many microseconds every `SENDS_PER_YIELD` (256) iterations on top of the unconditional `yield_now().await`. Bluefin has no on-wire congestion control (bluefin-architecture §8) so on a contended 3-vCPU hosted runner a tight client loop can outrun the receiver's drain rate, overflow the kernel UDP buffer, and trigger packet loss the protocol can't recover from. 50 µs / 256 sends ≈ ~0.2 µs/send overhead — negligible vs. the ~1 GB/s peak (~1.5 µs/1500 B packet) — but gives the receiver a real scheduling window every batch. Production unaffected unless set. |
