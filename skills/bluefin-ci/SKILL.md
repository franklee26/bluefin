...
| `BLUEFIN_CLIENT_SEND_THROTTLE_EVERY` | `256` (default, matches `SENDS_PER_YIELD`) / `2048` (workflow) | Cadence for the throttle above (number of sends between sleeps). Only takes effect when `BLUEFIN_CLIENT_SEND_THROTTLE_US` > 0. Empirical sweep on Apple-silicon dev (`25 µs` sleep, ~1.44 GB/s unthrottled baseline): `every 256 → 250 MB/s`, `every 1024 → 650 MB/s`, `every 2048 → 1.0 GB/s` (~70 % of baseline), `every 4096 → 1.2 GB/s`. Workflow picks `2048` as a balance — paces bursts hard enough to stop receiver overruns (good_conns went from flaky → 10/10) while still letting the bench measure ~70 % of the runner's ceiling. The original `256` was empirically too aggressive: good_conns 10/10 but mean throughput collapsed to ~0.17 GB/s, defeating the "bench" purpose. |
