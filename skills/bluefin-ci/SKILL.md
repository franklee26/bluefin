...
| `BLUEFIN_BENCH_FLOOR_MAX_PEAK_GBPS` | `0.10` (script) / `1.30` (workflow) | Minimum of the *maximum* observed `peak gb/s` across **GOOD conns only** (filtered). Below this → FAIL. Rebaselined when the client-side throttle (`BLUEFIN_CLIENT_SEND_THROTTLE_EVERY=2048`) capped the runner at ~70 % of its unconstrained ceiling. |
| `BLUEFIN_BENCH_FLOOR_GOOD_CONNS`    | `6` (script) / `3` (workflow) | Minimum count of "good conns" out of `N_RUNS * N_CONNS` total trials. Below this → FAIL. The threshold for "good" is `BLUEFIN_BENCH_GOOD_CONN_MIN_BYTES`. CI is set to **3 of 10** and acts as a **light liveness gate**: ≥3 conns must complete the full 700 MB payload end-to-end across 5 runs. Tightened from `2` once the client throttle stabilised good_conns to ≥10/10 on healthy runners; `3` still leaves comfortable headroom for the long left tail of hosted-runner allocation noise while catching real handshake/end-to-end regressions. Throughput regressions are caught separately by `FLOOR_MEAN_AVG_GBPS` / `FLOOR_MAX_PEAK_GBPS` (both GOOD-only). |
...
| `BLUEFIN_CLIENT_SEND_THROTTLE_US` | `0` (production / dev) / `25` (workflow) | Inter-batch pacing knob read by [`bluefin/src/bin/client.rs`](../../bluefin/src/bin/client.rs). When >0, the client send loop sleeps that many microseconds every `BLUEFIN_CLIENT_SEND_THROTTLE_EVERY` iterations. Bluefin has no on-wire congestion control (bluefin-architecture §8) so on a contended 3-vCPU hosted runner a tight client loop can outrun the receiver's drain rate, overflow the kernel UDP buffer, and trigger packet loss the protocol can't recover from. **Important:** `tokio::time::sleep` has a ~1 ms timer-wheel floor on most platforms, so the *cadence* (`THROTTLE_EVERY`) matters more than the µs value. Effective rate cap ≈ `THROTTLE_EVERY × 1500 B / 1 ms`. Production unaffected unless set. |
| `BLUEFIN_CLIENT_SEND_THROTTLE_EVERY` | `256` (default, matches `SENDS_PER_YIELD`) / `3000` (workflow) | Cadence for the throttle above (number of sends between sleeps). Only takes effect when `BLUEFIN_CLIENT_SEND_THROTTLE_US` > 0. Empirical sweep on Apple-silicon dev (`25 µs` sleep, ~1.44 GB/s unthrottled baseline): `every 256 → 250 MB/s`, `every 1024 → 650 MB/s`, `every 2048 → 1.0 GB/s`, `every 4096 → 1.2 GB/s`. Workflow picks `3000` (interpolates to ~1.1 GB/s, ~80 % of baseline) as a balance — paces bursts hard enough to stop receiver overruns (good_conns went from flaky → 10/10) while still letting the bench measure most of the runner's ceiling. The original `256` was empirically too aggressive: good_conns 10/10 but mean throughput collapsed to ~0.17 GB/s, defeating the "bench" purpose. |
...
| `BLUEFIN_RECV_IDLE_TIMEOUT_SECS` | unset (production = `2`) / `20` (workflow) | Server's per-conn recv-idle deadline in seconds, read once at startup by [`bluefin/src/bin/server.rs`](../../bluefin/src/bin/server.rs). With the FIN/FinAck close (bluefin-protocol §10bis) wired into the bench bins, a *healthy* client now ends each conn with `BluefinConnection::close()` → the server's `recv_bytes` returns `Ok(0)` and the conn exits cleanly without waiting on this timer. The recv-idle deadline is retained as a **safety net for crashed / SIGKILL'd clients only**; CI's 20 s value is sized to absorb mid-stream scheduling stalls on hosted runners (5–10 s observed in practice), not the steady-state transfer time. The server's FINAL `avg gb/s` subtracts this tail from the divisor **only on the non-clean-EOF path** so clean exits don't get the (now-unused) correction. |
...
With Option C, `good_conns` returns to its original semantic ("delivered the full payload") and the floor of 3/10 acts as a **light liveness gate** (≥ 3 of 10 conn-trials must complete end-to-end). Throughput regressions are caught separately and independently by the GOOD-only `FLOOR_MEAN_AVG_GBPS` / `FLOOR_MAX_PEAK_GBPS` floors. The deliberately loose `good_conns` floor exists because hosted-runner allocation noise has a long left tail that no amount of retry/timeout tuning fully eliminates — even with the client throttle, which targets receiver-side overruns rather than CPU starvation.

**Server FINAL avg correction**: `bluefin/src/bin/server.rs` subtracts `recv_idle` from `now.elapsed()` when computing the FINAL avg **only when the conn did NOT exit via clean EOF** (i.e. the recv-idle timer actually fired). On the clean-EOF path (peer called `close()`, `recv_bytes` returned `Ok(0)`) the raw elapsed is used as-is — there's no idle tail to subtract. On the truncating path, a conn that delivered 750 MB in 2.5 s of real transfer + 10 s idle still reports `avg = 750 MB / 2.5 s = 300 MB/s`, not `750 MB / 12.5 s = 60 MB/s`. Without this correction, bumping the timeout would have dragged mean_avg into the noise floor on the hosted-runner failure path.
...
| `max peak gb/s` (good-conns only) | **1.30** | Rebaselined 2026-05-15 after the client throttle (`BLUEFIN_CLIENT_SEND_THROTTLE_US=25` / `THROTTLE_EVERY=2048`) was added to absorb receiver-overrun packet loss. The throttle caps the runner at ~70 % of its unconstrained ceiling; `1.30` is conservative against that cap. **Regression signal for burst rate on conns that actually transferred data.** |
| `good conns` (≥ 700 MB each) | **3** of 10 | Liveness gate. Hosted-runner allocation has a long left tail; the bench script auto-retries any run that yields 0 GOOD conns (`BLUEFIN_BENCH_RUN_RETRIES_ON_ZERO=2`), which absorbs the catastrophic-allocation tail. Floor tightened from `2` to `3` (2026-05-15) once the client throttle stabilised good_conns to ≥10/10 on healthy runners. |
...
4. **`good_conns` had to be reframed.** It was originally trying to gate two things — protocol liveness AND runner reliability — but the second is fundamentally adversarial noise. It's now a **light liveness gate** (≥ 3 of 10) that pairs with the GOOD-only throughput floors. The throughput floors gate against real perf regressions; `good_conns` only catches the pathological case where bluefin can't complete *any* end-to-end transfer.
...
| **Reproduce the workflow exactly** | Same as above plus `BLUEFIN_BENCH_FLOOR_MEAN_AVG_GBPS=0.40 BLUEFIN_BENCH_FLOOR_MAX_PEAK_GBPS=1.30 BLUEFIN_BENCH_FLOOR_GOOD_CONNS=3 BLUEFIN_BENCH_GOOD_CONN_MIN_BYTES=700000000 BLUEFIN_SOCKET_RCVBUF=8388608 BLUEFIN_SOCKET_SNDBUF=8388608 BLUEFIN_NUM_SENDS=500000 BLUEFIN_RECV_IDLE_TIMEOUT_SECS=20 BLUEFIN_NUM_READER_WORKERS=2 BLUEFIN_CLIENT_SEND_THROTTLE_US=25 BLUEFIN_CLIENT_SEND_THROTTLE_EVERY=3000 BLUEFIN_BENCH_RUN_RETRIES_ON_ZERO=2 BLUEFIN_BENCH_RUN_RETRIES_ON_PARTIAL=0 BLUEFIN_BENCH_RUN_RETRY_BACKOFF_SECS=5 BLUEFIN_BENCH_SUMMARY_MD=/tmp/x.md ./bench_ci.sh -r 5 -n 2 --skip-build`. |
