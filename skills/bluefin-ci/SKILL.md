...
| `BLUEFIN_BENCH_FLOOR_GOOD_CONNS`    | `6` (script) / `2` (workflow) | Minimum count of "good conns" out of `N_RUNS * N_CONNS` total trials. Below this → FAIL. The threshold for "good" is `BLUEFIN_BENCH_GOOD_CONN_MIN_BYTES`. CI is set low (2 of 10) and acts as a **light liveness gate**: ≥ 2 conns must complete the full 700 MB payload end-to-end across 5 runs. Throughput regressions are caught separately by `FLOOR_MEAN_AVG_GBPS` / `FLOOR_MAX_PEAK_GBPS` (both GOOD-only). The looseness here exists because hosted macos-latest has a long left tail of allocation noise that no amount of retry/timeout tuning fully eliminates. |
...
With Option C, `good_conns` returns to its original semantic ("delivered the full payload") and the floor of 2/10 acts as a **light liveness gate** (≥ 2 of 10 conn-trials must complete end-to-end). Throughput regressions are caught separately and independently by the GOOD-only `FLOOR_MEAN_AVG_GBPS` / `FLOOR_MAX_PEAK_GBPS` floors. The deliberately loose `good_conns` floor exists because hosted-runner allocation noise has a long left tail that no amount of retry/timeout tuning fully eliminates.
