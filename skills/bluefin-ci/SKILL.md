...
| `BLUEFIN_BENCH_RUN_RETRIES` | `0` (script) / `1` (workflow) | Maximum extra attempts per run when the run yields 0 GOOD conns. Defaults to no retry. CI sets 1 because hosted macos-latest occasionally allocates a runner so contended that both bench peers get starved on the first attempt; a single retry after a short cooldown clears it ~99 % of the time. Only the final attempt's data is committed to the aggregate; earlier attempts' logs are kept on disk under their own `attempt_*` directories but are NOT parsed. |
