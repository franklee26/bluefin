...
11. ~~**Server handshake race (correctness, not throughput, but visible in benches)**~~ — **FIXED 2026-05.** A shared `HelloState` struct ([`net/mod.rs`](../../bluefin/src/net/mod.rs)) now queues `ClientHello` packets that arrive before an `accept()` slot is ready (bounded queue, cap 64). `accept()` drains the queue before blocking on the `HandshakeConnectionBuffer`. The `ReaderTxChannel` and `accept()` coordinate under a single mutex with O(1) `VecDeque` operations — no TOCTOU race, no client-side stagger needed. All stagger workarounds removed from `client.rs`, `bench_two_process.sh` (default stagger now 0), `bench_run_with_timeout.sh`, and integration tests.
...
- **Insert-then-handshake split in `BluefinServer`/`BluefinClient`** — `pending_accept_ids` is `VecDeque<u32>` consumed FIFO via `pop_front()` (was `Vec::pop()`/LIFO, which mis-routed hellos; then `Vec::remove(0)` O(n); now `VecDeque::pop_front()` O(1)).
- **Server accepts all connections then spawns processing tasks** (was inline spawn-per-accept, which mis-ordered handshakes).
- **Server-side hello queue** (`HelloState` in [`net/mod.rs`](../../bluefin/src/net/mod.rs)) buffers `ClientHello` packets arriving before `accept()` (cap 64). Client-side stagger delays removed.
...
| Goal | Script | Why |
|---|---|---|
| One-off ad-hoc throughput measurement, N connections | [`bench_two_process.sh`](../../bench_two_process.sh) | Has `-n`, `--stagger`, `--retry`, `--skip-build`; preserves `bench_logs/<ts>/attempt_<N>/` |
